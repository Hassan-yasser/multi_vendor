    ...
    <div class="p-3 d-flex align-items-center bg-body-secondary justify-content-between flex-wrap gap-2">
      ...
      <div class="d-flex gap-2 justify-content-end">
        @if (auth()->user()?->is_admin)
          <a href="{{ route('sub_categories.edit', $subCategory) }}" class="btn btn-sm btn-primary">{{ __('Edit') }}</a>
        @endif
        <a href="{{ route('sub_categories.index') }}" class="btn btn-sm btn-outline-secondary">{{ __('Back') }}</a>
      </div>
    </div>
    <div class="card-body">
      <dl class="row mb-0">
