...
<x-page-heading title="Edit Inventory" :breadcrumb-items="[['label'=>'Dashboard','url'=>route('dashboard')],['label'=>'Inventory','url'=>route('inventory.index')],['label'=>'Edit']]"/>
@endsection
@section('content')
    ...
    <form method="POST" action="{{route('inventory.update',$inventory)}}">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="address" class="form-control @error('address') is-invalid @enderror" value="{{old('address',$inventory->address)}}">
        ...
        <div class="col-12">
          ...
          <div class="row g-3" id="productsGrid">
            ...
            @php
            $isSelected = array_key_exists($p->id, $inventoryProducts);
            $qty = $isSelected ? $inventoryProducts[$p->id] : 0;
            ...
            <div class="col-md-6 col-lg-4">
              <div class="card product-card cursor-pointer h-100 {{$isSelected?'selected':''}}" onclick="toggleProductCard(this,{{$p->id}})">
                <div class="card-body p-3">
                  <input type="checkbox" name="products[]" value="{{$p->id}}" class="form-check-input position-absolute top-0 end-0 m-2 product-check" style="z-index:10" {{$isSelected?'checked':''}}>
                  ...
                  <input type="number" name="quantities[{{$p->id}}]" class="form-control form-control-sm product-qty mt-2" value="{{$qty}}" min="0" placeholder="Quantity">
                </div>
              </div>
            </div>
            @empty
